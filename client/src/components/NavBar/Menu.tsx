...
const Menu: React.FC<MenuProps> = ({ setPage, logout, setDropdown, dropdown, page }) => {

    const ref = React.useRef(null);

    useOnClickOutside(ref, () => setDropdown(false));

    const dr = (e: React.MouseEvent) => {
        e.stopPropagation()
    ...
    return (
        <div className="menu">
            ...
            <div className="gear" onClick={(e) => dr(e)} ref={ref}>
                <div id="gear-svg">
                    <svg className={page == 'account' || dropdown ? "settings-active" : "settings"} width="20" height="20" viewBox="0 0 20 20">
                ...
                <div className={dropdown ? "dropdown-active" : "dropdown"} >
                    <div className="dropdown-btn" onClick={openAccount}>Account</div>
                    <div className="dropdown-btn" >Stop</div>
                    <div className="dropdown-btn" onClick={logout}>Log Out</div>
                </div>
            </div>
            <svg className={page == 'playList' ? "playList-active" : "playList"} viewBox="0 0 256 256" >
